...
func Example2PromiseResultTestingCallback() {
	...
	if sdkErr != nil {
		...
		env.LogString("SDK method success, data length: " + types.IntToString(len(sdkData)))
		if len(sdkData) > 0 {
	...
	result1, err1 := promiseBuilder.GetPromiseResult(0)
	if err1 != nil {
		env.LogString("Contract method failed: " + err1.Error())
	} else {
		env.LogString("Contract method success: " + strconv.FormatBool(result1.Success))
		env.LogString("Contract method status: " + types.IntToString(int(result1.StatusCode)))
	...
	result2, err2 := promiseBuilder.GetPromiseResultSafe(0)
	if err2 != nil {
		env.LogString("Safe method failed: " + err2.Error())
	} else {
		env.LogString("Safe method success: " + strconv.FormatBool(result2.Success))
		env.LogString("Safe method status: " + types.IntToString(int(result2.StatusCode)))
	...
	status3, data3, err3 := promiseBuilder.GetPromiseResultWithStatus(0)
	if err3 != nil {
		env.LogString("Low-level method failed: " + err3.Error())
	} else {
		env.LogString("Low-level method status: " + types.IntToString(int(status3)))
		if len(data3) > 0 {
	...
	env.LogString("Direct promise result status: " + types.IntToString(int(directStatus)))

	registerLength := env.NearBlockchainImports.RegisterLen(env.AtomicOpRegister)
	env.LogString("Register length: " + types.IntToString(int(registerLength)))

	if registerLength > 0 {
		directData, directErr := promiseBuilder.ReadRegisterSafeWithFallback(env.AtomicOpRegister)
		if directErr != nil {
			env.LogString("Direct register read failed: " + directErr.Error())
		} else {
			env.LogString("Direct register read success, data length: " + types.IntToString(len(directData)))
			if len(directData) > 0 {
